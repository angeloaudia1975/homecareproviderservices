...
| Territory Map | src/admin/map.html | 2.0 | OSM pins + routing + home base (1.x); **Phase 2 (2.0)**: staff email/password login; pins scoped to the rep's book (server-side, default-deny in geocode-api); geocoding setup is President-only; reps without travel access are blocked from the map. |
