...
exports.handler = async (event)=>{
  try{
    ...
    if(b.action==="set_automation_config"){
      if(me.role!=="president") return json(403,{error:"President only"});
      const patch=b.patch||{}; const allow=new Set(["engine_enabled","email_enabled","cap_per_7d","min_gap_hours","dormant_months","overdue_mult","overdue_min_gap_months","quiet_weekends","business_hours","timezone","windows","templates_enabled","queue_ttl_hours","exclude_manufacturers","exclude_dealers","restricted_relationships","reports_enabled","report_recipients"]);
